...
public sealed class OperationsQueriesTests
{
    ...
    public void Alert_boundaries_do_not_trigger_until_the_threshold_is_exceeded()
    {
        ...
        var telemetry = new[]
        {
            Row(timestamp, timestamp + AlertRules.LateArrivalThreshold, 0, AlertRules.HighWindMinimumMs - 1, AlertRules.GearboxCriticalTemperatureC),
            Row(timestamp + AlertRules.ExpectedCadence, timestamp + AlertRules.ExpectedCadence, 1, 9, 99),
        };
        var queries = new OperationsQueries(CreateRepository(telemetry));
    ...
    public void Farm_without_telemetry_is_returned_but_not_in_dashboard()
    {
        var queries = new OperationsQueries(new InMemoryRepository(new OperationsData(
            ...
            [Row(DateTimeOffset.UnixEpoch, DateTimeOffset.UnixEpoch, 2, 5, 50)])));
    ...
    {
        var queries = new OperationsQueries(CreateRepository([]));
